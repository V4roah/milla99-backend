...
class DriverSearchService:
    ...
    def find_far_busy_drivers(
        ...
        """
        Busca conductores ocupados (ON_THE_WAY, ARRIVED, TRAVELLING) que están lejos del cliente (para análisis).
        Solo considera conductores que NO tienen solicitudes pendientes.
        ...
        try:
            ...
            for driver in busy_drivers:
                ...
                # Incluir solo los que exceden alguna validación
                if (distance > max_distance or
                    transit_time > max_transit_time or
                    ...
                    if distance > max_distance:
                        invalid_busy_drivers[-1]["reasons"].append(
                            f"Distancia {distance:.2f}km > {max_distance}km")
                    if transit_time > max_transit_time:
                        invalid_busy_drivers[-1]["reasons"].append(
                            f"Tránsito {transit_time:.2f}min > {max_transit_time}min")
                    if total_time > max_wait_time:
                        invalid_busy_drivers[-1]["reasons"].append(
                            f"Tiempo total {total_time:.2f}min > {max_wait_time}min")
